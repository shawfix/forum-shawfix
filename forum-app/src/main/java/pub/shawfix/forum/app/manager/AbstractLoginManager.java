...
import pub.shawfix.forum.api.request.user.UserBaseLoginRequest;
...
public class AbstractLoginManager {
    ...
    protected void cacheLoginUser(String token, User user) {
        ...
        // 重新保存缓存
        cacheService.setAndExpire(CacheBizTypeEn.USER_LOGIN_TOKEN, String.valueOf(user.getId()), token, USER_LOGIN_TOKEN_EXPIRE_TIMEOUT);
        cacheService.setAndExpire(CacheBizTypeEn.USER_LOGIN_TOKEN, token, JSON.toJSONString(user), USER_LOGIN_TOKEN_EXPIRE_TIMEOUT);
    }
